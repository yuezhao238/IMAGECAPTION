...
wandb.init(project="imagecaption", name="ZhangModel")
...
config = Namespace(
    ...
    best_checkpoint = f'../model/ZhangModel/best_{dataset}.ckpt', # 验证集上表现最优的模型的路径
    last_checkpoint = f'../model/ZhangModel/last_{dataset}.ckpt', # 训练完成时的模型的路径
    beam_k = 5
)
...
if not os.path.exists('../model/ZhangModel'):
...
if checkpoint is None:
    model = ZhangModel(config.image_code_dim, vocab, config.word_dim, config.attention_dim, config.hidden_size, config.num_layers)
else:
