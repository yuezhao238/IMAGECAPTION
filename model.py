...
from layers.language import AttentionDecoder, GRUDecoder, TransformerDecoder, SelfAttentionDecoder
...
class ZhaoModel(nn.Module):
    ...
    def generate_by_beamsearch(self, images, beam_k, max_len):
        vocab_size = len(self.vocab)
        image_codes = self.encoder(images)
        image_codes = self.adapter(image_codes)
        texts = []
        device = images.device
        # 对每个图像样本执行束搜索
        for image_code in image_codes:
            ...
            start_token = start_token.unsqueeze(0).repeat(beam_k, 1)

            sequences = [[list(), 1.0]] * beam_k  # Each element in sequences is a tuple (seq, score)
            for _ in range(max_len):
                all_candidates = list()
                for i in range(len(sequences)):
                    seq, score = sequences[i]
                    if len(seq) > 0 and seq[-1] == self.vocab["<end>"]:
                        all_candidates.append(sequences[i])
                        continue
                    input_seq = torch.tensor(seq).unsqueeze(0).to(device)
                    input_seq = torch.cat([start_token, input_seq], dim=1)
                    output, _, _, _, _ = self.forward(image_code, input_seq, torch.tensor([len(seq) + 1]).to(device))
                    output = F.softmax(output, dim=-1)
                    output = output[:,-1,:]
                    top_k_scores, top_k_words = output[0].topk(beam_k)
                    for j in range(beam_k):
                        next_seq, next_score = seq + [top_k_words[j].item()], score * top_k_scores[j].item()
                        all_candidates.append((next_seq, next_score))
                ordered = sorted(all_candidates, key=lambda tup: tup[1], reverse=True)
                sequences = ordered[:beam_k]

            for seq, _ in sequences:
                text = [self.vocab.itos[idx] for idx in seq]
